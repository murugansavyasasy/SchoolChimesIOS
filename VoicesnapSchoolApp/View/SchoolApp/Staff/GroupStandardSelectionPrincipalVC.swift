//
//  GroupStandardSelectionPrincipalVC.swift
//  VoicesnapSchoolApp
//
...
class GroupStandardSelectionPrincipalVC: UIViewController ,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,UICollectionViewDataSource,Apidelegate {
    ...
    func callSendVoiceToGroupsAndStandards()
    {
        ...
        if DefaultsKeys.SelectInstantSchedule == 0 {
            ...
            print("requestStringerOL56tyegyh",requestStringer)
            ...
            let requestString = requestStringer.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed)
            print("recrevvquestStringerOL56ty",requestStringer)
    ...
    func callMeetingAPI()
    {
        ...
        self.showLoading()
        let baseUrlString = UserDefaults.standard.object(forKey:BASEURL) as? String
        let requestStringer = baseUrlString! + POST_MEETING_GROUP_STANDARDS
        let requestString = requestStringer.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed)
        ...
        apiCall.delegate = self;
        let myString = Util.convertDictionary(toString: self.selectedSchoolDictionary)
        apiCall.nsurlConnectionFunction(requestString, myString, "meeting")
    }
    func callSchoolEvents()
    {
        ...
        let myString = Util.convertDictionary(toString: self.selectedSchoolDictionary)
        print("myEventmyString",myString)
    ...
    func loadLanguageData(LangDict : NSDictionary, Language : String){
        ...
        entireButton.setTitle(commonStringNames.send_to_entire_school.translated() as? String, for: .normal)
